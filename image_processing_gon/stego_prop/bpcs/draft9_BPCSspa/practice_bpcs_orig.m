...
% [comp1, comp, prob1] = huff_enc(A);
siz_app= size(imo);
siz_app = siz_app(1)*siz_app(2)*siz_app(3)*8;
siz_app = (siz_app/8)*4;
if(numel(comp1)>=(siz_app))
    siz_enc = comp1(1:siz_app);
    siz_con = comp1(siz_app+1:end);
...
if(numel(mat2)~=0)
    ...
    %         cc = 64-numel(mat)+1;
    %     if(cc>3)
    %     add_val = [0,mat2, zeros(1,cc-3),de2bi(cc-3,3,'left-msb')];
    %     msg(msg_block+2,1:64) = add_val(:,:);
    %     else
    %     add_val = [0,mat2, zeros(1,cc)];
    %     msg(msg_block+2,1:64) = add_val(:,:);
    %     end
...
clearvars -except imo im_10 data siz_con comp1 plane dim
